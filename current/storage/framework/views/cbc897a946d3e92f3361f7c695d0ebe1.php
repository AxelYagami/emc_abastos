...
<head>
  ...
  <title><?php echo e($title ?? config('app.name','EMC Abastos')); ?></title>
  <meta name="description" content="<?php echo e($metaDescription ?? 'Central de Abastos - Los mejores productos al mejor precio'); ?>">
...
<body class="min-h-screen bg-gray-50 flex flex-col">
  ...
  <header class="bg-white border-b shadow-sm sticky top-0 z-50" x-data="miniCart()">
    <div class="max-w-7xl mx-auto px-4 py-3">
      <div class="flex items-center justify-between">
        <a href="<?php echo e(route('store.home')); ?>" class="flex items-center gap-2">
        ...
        <nav class="hidden md:flex items-center gap-6 text-sm font-medium">
          <a href="<?php echo e(route('store.home')); ?>" class="text-gray-700 hover:text-primary-600 transition">Tienda</a>
          <a href="<?php echo e(route('cart.index')); ?>" class="text-gray-700 hover:text-primary-600 transition">Carrito</a>
          <?php if(auth()->guard()->check()): ?>
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="text-gray-700 hover:text-primary-600 transition">Admin</a>
          <?php else: ?>
            <a href="<?php echo e(route('login')); ?>" class="text-gray-700 hover:text-primary-600 transition">Entrar</a>
          <?php endif; ?>
        </nav>

        <!-- Mini Carrito -->
        <a href="<?php echo e(route('cart.index')); ?>" class="flex items-center gap-2 bg-primary-50 hover:bg-primary-100 px-4 py-2 rounded-full transition">
  ...
  <div class="max-w-7xl mx-auto px-4 py-3">
    <button onclick="history.back() || window.location='<?php echo e(route('store.home')); ?>'" class="inline-flex items-center gap-2 text-gray-600 hover:text-primary-600 transition text-sm">
  ...
  <div class="max-w-7xl mx-auto px-4">
    <?php if(session('status') || session('ok')): ?>
      <div class="mb-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-green-800 flex items-center gap-2">
        ...
        <?php echo e(session('status') ?? session('ok')); ?>

      </div>
    <?php endif; ?>
    <?php if(session('error')): ?>
      <div class="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-red-800"><?php echo e(session('error')); ?></div>
    <?php endif; ?>
    <?php if($errors->any()): ?>
      <div class="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-red-800">
        <ul class="list-disc ml-5"><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><li><?php echo e($e); ?></li><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></ul>
      </div>
    <?php endif; ?>
  </div>
  ...
  <footer class="bg-secondary-800 text-white mt-auto">
    <div class="max-w-7xl mx-auto px-4 py-8">
      ...
      <div class="border-t border-gray-700 mt-6 pt-6 text-center text-gray-500 text-sm">
        © <?php echo e(date('Y')); ?> EMC Abastos · Desarrollado por <a href="https://iados.mx" class="text-primary-400 hover:underline">iaDoS.mx</a>
      </div>
    </div>
  </footer>
  ...
  <script>
    function miniCart() {
      return {
        itemCount: <?php echo e((function() { $cart = session('cart', []); if (!is_array($cart)) return 0; $sum = 0; foreach($cart as $item) { if (is_array($item) && isset($item['qty'])) $sum += (int)$item['qty']; } return $sum; })()); ?>,
        total: <?php echo e((function() { $cart = session('cart', []); if (!is_array($cart)) return 0; $sum = 0; foreach($cart as $item) { if (is_array($item) && isset($item['price'], $item['qty'])) $sum += (float)$item['price'] * (int)$item['qty']; } return $sum; })()); ?>
...
<?php /**PATH C:\sites\emc_abastos\current\resources\views/layouts/store.blade.php ENDPATH**/ ?>