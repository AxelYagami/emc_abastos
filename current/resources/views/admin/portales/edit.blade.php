    ...
    <form action="{{ route('admin.portales.update', $portal) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Dominio (legacy)</label>
                    <input type="text" name="dominio" value="{{ old('dominio', $portal->dominio) }}"
                           class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500"
                           placeholder="miportal.com">
                    <p class="text-xs text-gray-500 mt-1">Este campo se mantiene por compatibilidad</p>
                </div>
                <div class="md:col-span-2">
        ...
        <!-- Dominios / Portal -->
        <div x-data="{
            dominios: @js(old('dominios', $portal->dominios ?: [''])),
            addDominio() {
                this.dominios.push('');
            },
            removeDominio(index) {
            ...
            <h2 class="text-lg font-semibold text-gray-800 mb-2">Dominios del Portal</h2>
            <p class="text-sm text-gray-600 mb-4">
                Asigna uno o más dominios personalizados a este portal. Los visitantes que accedan por estos dominios verán automáticamente este portal.
            </p>

            <div class="space-y-3">
                <template x-for="(dominio, index) in dominios" :key="index">
                    <div class="flex gap-2">
                        <input type="text"
                               :name="'dominios[' + index + ']'"
                               x-model="dominios[index]"
                               placeholder="ejemplo: bodegadigital.com.mx"
                               class="flex-1 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500">
                        <button type="button"
                                @click="removeDominio(index)"
                                class="px-4 py-2 text-red-600 hover:bg-red-50 rounded-lg border border-red-200">
                ...
                <button type="button"
                        @click="addDominio()"
                        class="flex items-center gap-2 px-4 py-2 text-primary-600 hover:bg-primary-50 rounded-lg border border-primary-200 font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex gap-3">
                        ...
                        <div class="text-sm text-blue-800">
                            ...
                            <ul class="list-disc ml-4 space-y-1">
                                <li>Agrega todos los dominios que apuntan a este portal (incluyendo www y sin www)</li>
                                <li>Ejemplo: <code class="bg-blue-100 px-1 rounded">bodegadigital.com.mx</code> y <code class="bg-blue-100 px-1 rounded">www.bodegadigital.com.mx</code></li>
                                <li>Los dominios deben estar configurados en tu servidor DNS apuntando a este servidor</li>
                            </ul>
                        </div>
                    </div>
                </div>
