    ...
    <div class="space-y-3">
        ...
        @empty
            <div class="text-center py-12 text-gray-400">
                ...
                <p class="text-lg font-medium text-gray-500 mb-1">Sin ordenes</p>
                <p class="text-sm">
                    @if($grupo === 'pendientes')
                        No hay ordenes pendientes para {{ \Carbon\Carbon::parse($fecha)->format('d/m/Y') }}
                    @elseif($grupo === 'proceso')
                        No hay ordenes en preparacion
                    @elseif($grupo === 'listas')
                        No hay ordenes listas para entregar
                    @elseif($grupo === 'finalizadas')
                        No hay ordenes finalizadas para esta fecha
                    @else
                        No hay ordenes con estos filtros
                    @endif
                </p>
                <p class="text-xs mt-2">Las ordenes del carrito online apareceran aqui automaticamente</p>
            </div>
        @endforelse
    </div>
